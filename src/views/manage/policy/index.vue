...
<!--      <el-form-item label-width="180" label="策略方案，如：80代表8折" prop="discount">-->
<!--        <el-input-->
...
<!--      </el-form-item>-->
    ...
    <el-dialog title="策略详情" v-model="policyOpenInfo" width="500px" append-to-body>
<!--      el-input添加标题-->
      <div style="display: flex ; align-items: center">
        <span style="width: 80px;font-weight: bold">策略名称 : </span>
        <el-input v-model="form.policyName" readonly/>
      </div>

      <div>
        <span style="font-weight: bold">包含设备 ： </span>
<!--        el-table显示设备详情-->
        <el-table :data="vmList" style="width: 100%" border>
          <el-table-column label="序号" align="center" type="index" width="50"/>
 ...
 */
//参数设置
const vmQueryParams = ref({
  pageNum: 1,
  pageSize: 1000,
});
//定义策略详情对话框开关
const policyOpenInfo = ref(false);
const vmList = ref([]);



function getPolicyInfo(row) {
  policyOpenInfo.value = true;
  form.value = row;
  vmQueryParams.value.policyId = row.policyId;
  listVm(vmQueryParams.value).then(response => {
