<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" @submit.native.prevent v-show="showSearch" label-width="68px">
    ...
    <!-- 查看详情对话框 -->
    <el-dialog v-if="nodeVoList.length" title="区域详情" v-model="regionInfoOpen" width="500px" append-to-body>
      <el-form-item label="区域名称" prop="regionName">
        <el-input v-model="form.regionName" placeholder="请输入区域名称" disabled />
      </el-form-item>
<!--      <label>包含点位：</label>-->
      <div>
        <el-table  :data="nodeVoList" style="width: 100%">
...
/* 查看详情操作按钮 */
const nodeVoList = ref([
  { id: 1, nodeName: 'Node 1', vmcount: 10 },
  { id: 2, nodeName: 'Node 2', vmcount: 20 },
  { id: 3, nodeName: 'Node 3', vmcount: 30 }
]);
...
async function getRegionInfo(row){
  ...
  // 查看点位列表
  nodeParams.regionId=row.id
  await listNode(nodeParams).then(response => {
      nodeVoList.value = response.rows;
  });
